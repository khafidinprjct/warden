...
def writer_quiet(p: str | Path, open_writers: list[str] | None = None, min_age_s: int = 90) -> tuple[bool, str]:
    ...
    if open_writers and p.name in open_writers:
        return False, "terdaftar sedang ditulis (open_writers)"
    if p.with_suffix(p.suffix + ".tmp").exists() or p.with_suffix(p.suffix + ".partial").exists():
        return False, "ada berkas .tmp/.partial"
    import time
    if time.time() - p.stat().st_mtime < min_age_s:
        return False, f"mtime < {min_age_s} dtk"
    return True, ""
...
def verify_csv(p: Path, expect: dict, r: VerifyResult) -> None:
    ...
    r.add("csv_parse", len(rows) >= 1, "kosong")
    if not rows:
        return
...
def verify_npz(p: Path, expect: dict, r: VerifyResult) -> None:
    ...
    for k in expect.get("keys", []):
        r.add(f"npz_key:{k}", k in keys, "kunci hilang")
    bad = [k for k in keys if z[k].dtype.kind == "f" and not np.isfinite(z[k]).all()]
    r.add("npz_finite", not bad, f"non-finite di {bad[:3]}")
...
def verify_parquet(p: Path, expect: dict, r: VerifyResult) -> None:
    ...
    with open(p, "rb") as f:
        f.seek(-4, 2); r.add("parquet_footer", f.read(4) == b"PAR1", "footer PAR1 hilang (terpotong)")
    try:
...
def verify_torch(p: Path, expect: dict, r: VerifyResult) -> None:
    ...
    with open(p, "rb") as f:
        r.add("torch_zip_magic", f.read(2) == b"PK", "bukan format zip torch ≥1.6 (terpotong/format lama)")
    try:
        with zipfile.ZipFile(p) as z:
            bad = z.testzip(); r.add("torch_zip_integrity", bad is None, f"entri rusak: {bad}")
    except Exception as e:
    ...
    try:
        ...
        r.add("torch_load", True, "torch tidak terpasang di verifier — hanya integritas zip"); return
    except Exception as e:
        r.add("torch_load", False, str(e)[:100]); return
    sd = obj.get("state_dict", obj) if isinstance(obj, dict) else {}
    for k in expect.get("keys", []):
        r.add(f"torch_key:{k}", k in obj if isinstance(obj, dict) else False, "kunci hilang")
    import torch as _t
    ...
    r.add("torch_finite_sample", not nonfinite, f"{len(nonfinite)} tensor non-finite")
...
def verify(path: str | Path, expect: dict | None = None, declared_sha256: str = "", prev_sha256: str = "",
    ...
    if not p.exists():
        r.add("exists", False, "berkas tidak ada"); return r
    ...
    if declared_sha256:
        r.add("sha256_matches_declared", r.sha256 == declared_sha256, "sha256 ≠ yang dideklarasikan RUN_FIN")
    if prev_sha256:
        r.add("not_identical_to_previous", r.sha256 != prev_sha256, "identik dengan artefak sebelumnya (salinan basi?)")
    if expect.get("bytes"):
        tol = expect.get("tol", 0.10); lo, hi = expect["bytes"] * (1 - tol), expect["bytes"] * (1 + tol)
        r.add("size_vs_expect", lo <= r.bytes <= hi, f"{r.bytes} di luar [{int(lo)},{int(hi)}] ({r.bytes/expect['bytes']*100:.0f}% dari ekspektasi)")
    ...
    if fn:
        ...
        r.add("plugin", True, "tipe tidak dikenal — hanya ukuran+sha")
    return r
