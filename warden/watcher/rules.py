...
def evaluate(f: Facts) -> list[Finding]:
    ...
    if inst and job and job.status == JobStatus.RUNNING and inst.status == InstanceStatus.TERMINATED \
        ...
        out.append(Finding("preempted" if preempted else "stopped_external", "critical",
                           f"{inst.ref} TERMINATED tanpa RUN_FIN ({'preempt' if preempted else 'dihentikan dari luar'}); job {job.job_id} fase {job.phase}",
    ...
    if f.done_legacy and f.run_fin is None:
        out.append(Finding("done_without_exit", "warning", f"{job.job_id if job else '?'}: marker DONE tanpa RUN_FIN/exit code — TIDAK diterima",
                           f"donenoexit:{f.done_legacy.job_id}:{f.done_legacy.run_id}", suggested_action="notify"))
    if f.run_fin and not f.run_fin.valid:
        out.append(Finding("marker_invalid", "warning", f"RUN_FIN tidak sah: {f.run_fin.invalid_reason}",
    ...
    if f.run_fin and f.run_fin.valid and f.run_fin.exit_code not in (None, 0):
        out.append(Finding("run_fin_nonzero", "critical", f"job {f.run_fin.job_id} berakhir exit={f.run_fin.exit_code}",
    ...
    if f.run_fin and f.run_fin.valid and f.run_fin.exit_code == 0 and job and job.status != JobStatus.COMPLETE:
        out.append(Finding("fin_ok_pending_verify", "info", f"job {f.run_fin.job_id} exit 0 — menunggu verifikasi artefak",
    ...
    if inst and inst.status == InstanceStatus.RUNNING and job and job.status == JobStatus.RUNNING and hb and f.run_fin is None:
        ...
        if age > thr and quiet:
            out.append(Finding("stuck", "critical", f"{job.job_id}: denyut basi {age.total_seconds()/60:.0f} mnt (> {thr.total_seconds()/60:.0f}) DAN mesin diam",
            ...
            out.append(Finding("slow", "warning", f"{job.job_id}: denyut basi tapi mesin sibuk (lambat, bukan macet)",
        ...
        if age > timedelta(minutes=3) and hb.synthetic is False and not hb.procs and hb.cpu_pct is None:
            out.append(Finding("harness_dead", "critical", f"{job.job_id}: harness tidak berdenyut {age.total_seconds()/60:.0f} mnt saat mesin RUNNING",
    ...
    if hb and hb.disk_avail_gb is not None:
        ...
        if hb.disk_avail_gb < max(2 * need, 5.0):
            ...
            out.append(Finding("disk_low", sev, f"{hb.job_id}: disk sisa {hb.disk_avail_gb:.1f} GB (butuh ≥ {max(2*need,5.0):.1f})",
    ...
    if hb and hb.procs and job and job.command:
        ...
        if len(mains) > 1:
            out.append(Finding("dup_process", "critical", f"{job.job_id}: {len(mains)} proses entrypoint berjalan bersamaan",
    ...
    if inst and inst.status == InstanceStatus.RUNNING and f.boot_age_min > g["boot_grace_minutes"]:
        ...
        if not f.in_ledger or (job is None) or job.status in (JobStatus.COMPLETE, JobStatus.FAILED, JobStatus.ABANDONED):
            if quiet and f.boot_age_min > g["orphan_grace_minutes"]:
                out.append(Finding("orphan", "warning", f"{inst.ref} hidup ${inst.hourly_price_usd:.3f}/jam tanpa job aktif",
            ...
            out.append(Finding("idle", "warning", f"{inst.ref}: job {job.job_id} idle ≥ {g['idle_grace_minutes']} mnt",
    ...
    if hb and hb.loss is not None and job and job.status == JobStatus.RUNNING:
        ...
        if math.isnan(hb.loss) or math.isinf(hb.loss):
            out.append(Finding("nan_loss", "critical", f"{job.job_id}: loss non-finite di step {hb.step}",
