...
def _log_lines(job_id: str) -> list[str]:
    from warden.agents.pipeline import read_log_tail
    return read_log_tail(job_id, n=100000)


def get_log_window(job_id: str, start_line: int, end_line: int) -> dict:
    """Return numbered log lines [start_line, end_line] (1-based, inclusive, at most 120 lines) of the job's log tail."""
    lines = _log_lines(job_id)
...
def search_log(job_id: str, pattern: str, max_hits: int = 20) -> dict:
    """Regex search over the job's log tail; returns matching line numbers and text (case-insensitive)."""
    lines = _log_lines(job_id)
    try:
    ...
    return {"job_id": job_id, "total_lines": len(lines), "hits": hits[: int(max_hits)], "n_hits": len(hits)}


def get_heartbeats(job_id: str, n: int = 40) -> dict:
    """Recent heartbeats (oldest first): ts, run_id, phase, step, loss, grad_norm, cpu_pct, gpu_util, disk_avail_gb, synthetic."""
    hbs = db.recent_heartbeats(job_id, min(int(n), 120))
...
def get_artifacts(job_id: str) -> dict:
    ...
    return {"job_id": job_id, "run_id": job.run_id, "status": str(job.status), "phase": job.phase, "last_step": job.last_step,
            "run_fin": ({"exit_code": fin.exit_code, "ts": fin.ts.isoformat(), "artifacts": [{"name": a["path"].split("/")[-1], "bytes": a.get("bytes"), "sha256": str(a.get("sha256", ""))[:12]} for a in fin.artifacts][:60],
...
def get_incident_history(job_id: str, n: int = 5) -> dict:
    ...
    return {"job_id": job_id, "incidents": out, "postmortems": memory.recall(job_id=job_id, n=5)}
...
async def investigate_async(job_id: str, incident_summary: str, instance_ref: str = "", findings: list[dict] | None = None,
                            model: str | None = None) -> tuple[str, list[dict], dict[str, Any]]:
    ...
    brief = {"job_id": job_id, "instance_ref": instance_ref, "incident": incident_summary, "deterministic_findings": (findings or [])[:6]}
    ...
    usage["model"] = model or settings.gemini_model; usage["tool_calls"] = len(tool_log)
