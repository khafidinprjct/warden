...
- ☐ C4 Gold evaluation set from real logs (NaN, OOM, wake-loop, kernel fallback, deps) — action accuracy ≥ 90 %, zero fabricated evidence, re-run on every prompt/model change.
...
- ◐ J2 ETTR per job (computed; surfaced in digest, not yet on the dashboard).
...
- ☐ K3 Ask Warden answers with citations and can propose an action through the same approval path.
- ☐ K4 Discord — last.
...
- ◐ L3 Daily digest (scheduled; ETTR included).
