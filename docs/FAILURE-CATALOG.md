...
## #44 · The committed Procfile was the dashboard's, so every core deploy shipped the dashboard (30 Aug 2026)
- **Symptom:** `gcloud run deploy warden-core --source .` reported success, revision `00039-w4v` served 100 % of traffic, and `GET /health` on the core URL returned `{"ok":true}`. Then `POST /jobs/launch` came back **200 with an HTML page** titled "Launch job · Warden" — the dashboard's form, not the core's JSON. Production core was down: no tick, no watcher, no ingest, no `/propose`.
- **Cause:** both services are built from one source tree and buildpacks read `Procfile` to decide what runs. Catalogue #39 forced the split into `Procfile` (core) and `Procfile.ui` (dashboard), and the dashboard deploy procedure is `cat Procfile.ui > Procfile && deploy && git checkout Procfile`. Yesterday the revert never happened, so the swapped file went into commit `badf28e` and the repository's own `Procfile` said `web: python -m warden.ui2.app`. The next core deploy did exactly what the file told it to.
- **Why the gate missed it:** `/health` is answered by **both** apps, so the check that catalogue #39 added for the dashboard was never enough for the core. A gate has to name something only the service under test has.
- **Fix:** `Procfile` restored to `uvicorn warden.main:app`; `tests/test_deployable_assets.py::test_procfile_is_the_core_entrypoint_not_the_dashboard` pins both files so the swap cannot be committed again; the runbook gates a core deploy on `curl -s $CORE/openapi.json | grep '"/tick"'`.
- **Cost:** production core down about eleven minutes, one wasted build, and a failed drill launch that read as a mysterious JSON decode error.
