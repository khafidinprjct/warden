...
State is in Firestore: incidents keep their verification spec, attempt count and remaining hypotheses, so a restart of warden-core resumes where it stopped. Re-deploy core: `gcloud run deploy warden-core --source .`. **The dashboard needs its own entrypoint**: buildpacks read `Procfile`, which is the core's, so `--source .` would deploy the core app under the `warden-ui` name (catalogue #39). Deploy it as `cat Procfile.ui > Procfile && gcloud run deploy warden-ui --source . && git checkout Procfile`, then verify with `curl -s <ui-url>/ | grep '<title>Overview'` — a 200 on `/health` proves nothing, both services answer it. **Put `Procfile` back, and gate the core the same way**: `curl -s <core-url>/openapi.json | grep '"/tick"'`. A core deploy made while `Procfile` still holds the dashboard's entrypoint serves the dashboard under the core's name and still answers `/health` (catalogue #44); `tests/test_deployable_assets.py` now fails if the swapped file is committed. Secrets: `warden-ingest-hmac` (rotate with `infra/rotate_hmac.py`, grace period keeps old signatures valid).
