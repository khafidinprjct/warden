...
- Koreksi pemilik: **STOP≠DELETE itu bersyarat**, bukan mutlak — berlaku saat mesin/bukti masih dibutuhkan, bukan untuk sisa uji.
- Pending pemilik: ekspor billing (J4); audit desktop UI (K1); Discord creds + video di akhir (K4).
...
5. **Compute**: izin pemilik sebelum memakai GPU/VM (+taksiran biaya); spot STOP bukan DELETE **selagi mesin/bukti masih dibutuhkan** (aturan bersyarat, bukan mutlak — koreksi pemilik 27 Agu; sisa uji yang sudah tak perlu boleh dihapus **atas perintah pemilik**, tak pernah atas inisiatif sendiri); `--no-boot-disk-auto-delete`; ledger dulu, mesin kemudian; verifikasi per sumber daya (jangan pangkas stderr).
